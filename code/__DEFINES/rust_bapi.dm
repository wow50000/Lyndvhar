...
/proc/__detect_bapi()
	if(world.system_type == UNIX)
		...
		if(fexists("./bapi/target/i686-pc-windows-msvc/release/bapi.dll"))
			return __bapi = "./bapi/target/i686-pc-windows-msvc/release/bapi.dll"
		// Also check the debug location if compiled without optimizations.
		if(fexists("./bapi/target/i686-pc-windows-msvc/debug/bapi.dll"))
			return __bapi = "./bapi/target/i686-pc-windows-msvc/debug/bapi.dll"
		// Then check in the current directory.
		if(fexists("./bapi.dll"))
