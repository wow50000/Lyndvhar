...
/obj/structure/vampire/bloodpool/attack_hand(mob/living/user)
	...
	if(user.mind.special_role != "Vampire Lord")
		return
	var/choice = input(user,"What to do?", "LYNDVHAR") as anything in useoptions|null
	switch(choice)
		if("Grow Power")
			if(lord.vamplevel == 4)
		...
		if("Shape Amulet")
			if(alert(user, "Craft a new amulet Cost:500","","Yes","No") == "Yes")
				...
				if(do_after(user, 100))
					...
					var/naming = input(user, "Select a name for the amulet.", "LYNDVHAR") as text|null
...
/datum/objective/vampirelord/infiltrate/two/check_completion()
	var/datum/game_mode/chaosmode/C = SSticker.mode
	var/list/noblejobs = list("Viscount", "Consort", "Heir", "Heiress", "Hand", "Steward")
	for(var/datum/mind/V in C.vampires)
...
/mob/dead/observer/rogue/arcaneeye/proc/scry_tele()
	...
	for(var/V in GLOB.sortedAreas)
		...
		if(!A.hidden)
			filtered += A
	var/area/thearea  = input("Area to jump to", "LYNDVHAR") as null|anything in filtered
