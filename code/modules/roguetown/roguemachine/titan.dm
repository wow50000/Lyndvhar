...
/obj/structure/roguemachine/titan/proc/give_tax_popup(mob/living/carbon/human/user)
	...
	if(newtax)
		...
		priority_announce("The new tax in Lyndvhar shall be [newtax] percent.", "The Generous Nobility Decrees", pick('sound/misc/royal_decree.ogg', 'sound/misc/royal_decree2.ogg'), "Captain")
...
/proc/make_outlaw(raw_message)
	if(raw_message in GLOB.outlawed_players)
		...
		priority_announce("[raw_message] is no longer an outlaw in the city of Lyndvhar.", "The [SSticker.rulertype] Decrees", 'sound/misc/royal_decree.ogg', "Captain")
		return FALSE
	var/found = FALSE
	for(var/mob/living/carbon/human/H in GLOB.player_list)
		if(H.real_name == raw_message)
	...
	if(!found)
		return FALSE
	GLOB.outlawed_players += raw_message
	priority_announce("[raw_message] has been declared an outlaw by the nobility and must be captured or slain.", "The [SSticker.rulertype] Decrees", 'sound/misc/royal_decree2.ogg', "Captain")
	return TRUE
...
/proc/purge_laws()
	...
	priority_announce("All laws of the city have been purged!", "LAWS PURGED", 'sound/misc/lawspurged.ogg', "Captain")
